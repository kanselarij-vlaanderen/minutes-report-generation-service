...
async function generatePdf(part: string, meeting: Meeting): Promise<FileMeta> {
  ...
  const html = renderMinutes(part, meeting);
...
app.get("/:id", async function (req, res) {
  try {
    ...
    const fileMeta = await generatePdf(sanitizedPart, meeting);
    res.send(fileMeta);
  } catch (e) {
    res.status(500);
    console.error(e);
